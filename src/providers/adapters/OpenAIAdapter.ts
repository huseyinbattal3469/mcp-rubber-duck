...
export class OpenAIAdapter implements ProviderAdapter {
  ...
  async chat(options: ChatOptions): Promise<ChatResponse> {
    const url = this.config.base_url.replace(/\/$/, '') + '/v1/chat/completions';
  ...
  async *chatStream(options: ChatOptions): AsyncGenerator<StreamChunk> {
    // OpenAI streaming: not implemented, fallback to single response
    const response = await this.chat(options);
    yield { content: response.content, done: true };
  }

  async listModels(): Promise<ModelInfo[]> {
    // OpenAI: use config.models or default
    if (this.config.models && this.config.models.length > 0) {
      return this.config.models.map(id => ({
        id,
        description: 'Configured model (OpenAI)',
      }));
    }
